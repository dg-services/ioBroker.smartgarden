...
class gardenaApiConnector{
					...
					switch (deviceType){
						...
						case 'MOWER': 
							...
							adapter.setObjectNotExists(locationID + '.' + deviceID + '.LastErrorCode', {
								type: 'state',
								common: {name: 'Device LastErrorCode', desc: 'Device LastErrorCode', type: 'string', read: true, write: false, role: 'text'},
								native: {}
							});
	...
	async execCommand(serviceID) {
		adapter.log.debug("execCommand function was called");
		...
		var body = JSON.stringify(myData);
		...
		try{
			...
			.then((response) => {
				if(response.status == '202'){
					adapter.log.debug('Command send successfull');
					return true;					
				} else {
					adapter.log.error('Command send failed: Status: '+response.status +" Failure: " + JSON.stringify(response) );	
					return false;					
				}	
			});
		} catch(error){
