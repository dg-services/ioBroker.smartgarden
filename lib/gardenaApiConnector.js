...
async function fctExecCommand(serviceID) {
	thisAdapter.log.silly('fctExecCommand() was called with serviceID: ' + serviceID);
...
async function fctGetWebsocket(){ 
	...
	ws.on('ping', function ping(data) {
		thisAdapter.log.silly('GardenaWebsocket Ping: ' + data);
	});
	ws.on('pong', function pong(data) {
		thisAdapter.log.silly('GardenaWebsocket Pong: ' + data);
	});
...
class gardenaApiConnector{
	...
	async login(){
		thisAdapter.log.silly('gardenaApiConnector.login() function was called');
	...
	async logout() {
		thisAdapter.log.silly('gardenaApiConnector.logout() function was called');
