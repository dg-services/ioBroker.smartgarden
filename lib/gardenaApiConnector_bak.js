...
async function doWebsocketPing(){
	adapter.log.debug('GardenaWebsocket Ping activated');
	while(websocketUp){
		await Sleep(60000);
		ws.ping('Ping');
	}
...
class gardenaApiConnector{
	...
	createIobrokerStateText(stateID, stateDeviceName, stateDeviceDesc, stateDeviceValue, stateRole, stateWriteable, statePossibleValues){
		adapter.setObjectNotExists(stateID + '.' + stateDeviceName, {
			type: 'state',
			common: {name: stateDeviceDesc, 
				desc: stateDeviceDesc, 
	...
	async getWebsocket(){ 
		...
		ws.on('close', function close(code, reason) {
			adapter.log.error('GardenaWebsocket disconnected: Code: ' + code + ' Reason: ' + reason );

			// Falls Error-Code 1000 == TokenUngültig == refresh
			if(code == 1000){
				refreshToken();
				ws.open();
			}
